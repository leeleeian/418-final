...
Performance goal: $>$4$\times$ on 8 cores (coarse) & \textbf{Not met yet} (1.44$\times$ at 8 threads); see Preliminary Results and Concerns. \\
\bottomrule
\end{tabular}
\end{table}
...
With fine-grained locking, we would replace the single per-book mutex with locks at smaller granularities inside the book. We would have to investigate the unit of locking though. We might consider the following strategies:
\begin{table}[h]
\centering
